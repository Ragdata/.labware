...
class FilterableDirectoryTree(DirectoryTree):
    ...
    def __init__(self, path: str, *, file_pattern: str = '*.*', **kwargs) -> None:
...
class OpenFileDialog(ModalScreen[str | bool]):
    ...
    def compose(self) -> ComposeResult:
        yield Vertical(
            ...
            Horizontal(
                Button(labels.OPEN, variant="primary", id="open_file"),
    ...
    def on_file_selected(self, event: DirectoryTree.FileSelected) -> None:
        ...
        self.query_one("#folder", Label).update(f"Folder name: {self.folder}")
        self.query_one("#filename", Input).value = str(event.path.name)
