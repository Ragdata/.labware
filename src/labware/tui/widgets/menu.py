...
from textual.events import Enter, Event, Key, Leave, Mount, MouseDown, MouseMove, MouseUp
...
class MenuItem(Static):
    ...
    def __init__(self, name: str = "", menu_action: str = "", callback: Optional[Callable[[str, str], None]] = None, **kwargs) -> None:
...
def get_child_index(parent: Widget, child: Widget):
...
class MenuScreen(ModalScreen[str]):
    ...
    def adjust_menu_region(self, region: Region) -> Region:
        return region.translate_inside(Region(0, 0, self.app.size.width, self.app.size.height - 1))
    ...
    async def context_menu(self, menu_id: Menu, offset: Offset, key: bool = False):
        ...
        try:
            ...
            self.app.pop_screen()
            return
        region = self.adjust_menu_region(Region(offset.x, offset.y, menu.virtual_region_with_margin.width, menu.virtual_region_with_margin.height))
    ...
    async def open_menu(self, menu_header: MenuHeader, key: bool = False):
        ...
        try:
            ...
            self.app.pop_screen()
            return
        region = self.adjust_menu_region(Region(offset[0], offset[1] + 1, menu.virtual_region_with_margin.width, menu.virtual_region_with_margin.height))
        menu.styles.offset = (region.x, region.y)
        self.menu_stack.append((menu, None))
        self.focus_menu(menu, key)
    ...
    def open_sub_menu(self, menu_item: MenuItem, menu_id: str, key: bool = False):
        ...
        try:
            ...
            return
        region = self.adjust_menu_region(Region(
    ...
    def pop_menu(self, key: bool = False):
